  ...
  return (
    <div className="space-y-4">
      {tasks.map((task) => (
        <div
          key={task.id}
          className={`${task.color} relative border-4 border-black shadow-[6px_6px_0px_rgba(0,0,0,1)] 
                      rounded-none p-4 transform hover:translate-x-1 hover:translate-y-1 
                      hover:shadow-[4px_4px_0px_rgba(0,0,0,1)] transition-all duration-200`}
        >
            ...
            <div className="flex-1">
              <p
                className={`text-xl font-bold ${
                  task.done ? "line-through text-gray-700" : "text-black"
                }`}
              >
                {task.title}
              </p>
              <p
                className={`text-sm font-bold mt-1 ${
                  task.done ? task.textColor : task.textColor
                }`}
              >
                {task.status}
              </p>
